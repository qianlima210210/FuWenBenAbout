...
@protocol QDIMEmptyViewLoadingViewProtocol <NSObject>
...
/**
 *  通用的空界面控件，支持显示 loading、标题和副标题提示语、占位图片，QDIMCommonViewController 内已集成一个 emptyView，无需额外添加。
 */
@interface QDIMEmptyView : UIView

// 布局顺序从上到下依次为：imageView, loadingView, textLabel, detailTextLabel, actionButton
@property(nonatomic, strong) UIView<QDIMEmptyViewLoadingViewProtocol> *loadingView;   // 此控件通过设置 loadingView.hidden 来控制 loadinView 的显示和隐藏，因此请确保你的loadingView 没有类似于 hidesWhenStopped = YES 之类会使 view.hidden 失效的属性
...
/**
 *  如果要继承QDIMEmptyView并添加新的子 view，则必须：
