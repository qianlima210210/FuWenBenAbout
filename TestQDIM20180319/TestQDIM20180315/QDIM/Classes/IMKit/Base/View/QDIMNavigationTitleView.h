...
@class QDIMNavigationTitleView;

NS_ASSUME_NONNULL_BEGIN

/// 设置title和subTitle的布局方式，默认是水平布局。
typedef NS_ENUM(NSInteger, QDIMNavigationTitleViewStyle) {
    QDIMNavigationTitleViewStyleDefault,                // 水平
    QDIMNavigationTitleViewStyleSubTitleVertical        // 垂直
};

/// 设置titleView的样式，默认没有任何修饰
typedef NS_ENUM(NSInteger, QDIMNavigationTitleViewAccessoryType) {
    QDIMNavigationTitleViewAccessoryTypeNone,                   // 默认
    QDIMNavigationTitleViewAccessoryTypeDisclosureIndicator     // 有下拉箭头
};

@protocol QDIMNavigationTitleViewDelegate <NSObject>

@optional

/**
 点击 titleView 后的回调，只需设置 titleView.userInteractionEnabled = YES 后即可使用。不过一般都用于配合 QDIMNavigationTitleViewAccessoryTypeDisclosureIndicator。
 
 @param titleView 被点击的 titleView
 @param isActive titleView 是否处于活跃状态（所谓的活跃，对应右边的箭头而言，就是点击后箭头向上的状态）
 */
- (void)didTouchTitleView:(QDIMNavigationTitleView *)titleView
...
- (void)didChangedActive:(BOOL)active
            forTitleView:(QDIMNavigationTitleView *)titleView;
 ...
 *
 *  支持主副标题；支持在左边显示loading，在右边显示accessoryView（如箭头）。
 *
 ...
 */
@interface QDIMNavigationTitleView : UIControl

@property(nonatomic, weak) id<QDIMNavigationTitleViewDelegate> delegate;
@property(nonatomic, assign) QDIMNavigationTitleViewStyle style;
 ...
 */
@property(nonatomic, assign) QDIMNavigationTitleViewAccessoryType accessoryType;
