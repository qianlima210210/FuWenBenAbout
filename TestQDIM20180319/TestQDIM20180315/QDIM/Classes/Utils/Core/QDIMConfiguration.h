...
@protocol QDIMConfigurationTemplateProtocol <NSObject>
...
@optional
/// 当返回 YES 时，启动 App 的时候 QDIMConfiguration 会自动应用这份配置表。但启动 App 时自动应用的配置表最多只允许一份，如果有多份则其他的会被忽略，需要在某些时机手动应用
- (BOOL)shouldApplyTemplateAutomatically;

@end

/**
 *  维护项目全局 UI 配置的单例，通过业务项目自己的 QDIMConfigurationTemplate 来为这个单例赋值，而业务代码里则通过 QDIMConfigurationMacros.h 文件里的宏来使用这些值。
 */
@interface QDIMConfiguration : NSObject
...
@property(nonatomic, assign) CGFloat            windowLevelQDIMAlertView;
@property(nonatomic, assign) CGFloat            windowLevelQDIMImagePreviewView;
