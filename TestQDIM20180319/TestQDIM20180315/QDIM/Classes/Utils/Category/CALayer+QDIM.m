...
@implementation CALayer (QDIM)

- (void)qdim_sendSublayerToBack:(CALayer *)sublayer {
...
- (void)qdim_bringSublayerToFront:(CALayer *)sublayer {
    if (sublayer.superlayer == self) {
        [sublayer removeFromSuperlayer];
...
- (void)qdim_removeDefaultAnimations {
...
+ (CAShapeLayer *)qdim_seperatorDashLayerWithLineLength:(NSInteger)lineLength
                                            lineSpacing:(NSInteger)lineSpacing
                                              lineWidth:(CGFloat)lineWidth
                                              lineColor:(CGColorRef)lineColor
                                           isHorizontal:(BOOL)isHorizontal {
    CAShapeLayer *layer = [CAShapeLayer layer];
    layer.fillColor = QDIM_UIColorClear.CGColor;
    ...
    if (isHorizontal) {
        ...
        CGPathAddLineToPoint(path, NULL, QDIM_SCREEN_WIDTH, lineWidth / 2);
    } else {
        CGPathMoveToPoint(path, NULL, lineWidth / 2, 0);
        CGPathAddLineToPoint(path, NULL, lineWidth / 2, QDIM_SCREEN_HEIGHT);
    }
    layer.path = path;
    CGPathRelease(path);
...
+ (CAShapeLayer *)qdim_seperatorDashLayerInHorizontal {
    CAShapeLayer *layer = [CAShapeLayer qdim_seperatorDashLayerWithLineLength:2 lineSpacing:2 lineWidth:QDIM_PixelOne lineColor:QDIM_UIColorSeparatorDashed.CGColor isHorizontal:YES];
    return layer;
}
+ (CAShapeLayer *)qdim_seperatorDashLayerInVertical {
    CAShapeLayer *layer = [CAShapeLayer qdim_seperatorDashLayerWithLineLength:2 lineSpacing:2 lineWidth:QDIM_PixelOne lineColor:QDIM_UIColorSeparatorDashed.CGColor isHorizontal:NO];
    return layer;
}

+ (CALayer *)qdim_separatorLayer {
    CALayer *layer = [CALayer layer];
    [layer qdim_removeDefaultAnimations];
    layer.backgroundColor = QDIM_UIColorSeparator.CGColor;
    layer.frame = CGRectMake(0, 0, 0, QDIM_PixelOne);
    return layer;
}

+ (CALayer *)qdim_separatorLayerForTableView {
    CALayer *layer = [self qdim_separatorLayer];
    layer.backgroundColor = QDIM_TableViewSeparatorColor.CGColor;
    return layer;
}
