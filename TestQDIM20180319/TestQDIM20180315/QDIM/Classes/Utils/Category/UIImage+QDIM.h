...
#define QDIM_CGContextInspectSize(size) [QDIMHelper inspectContextSize:size]

#ifdef DEBUG
#define QDIM_CGContextInspectContext(context) [QDIMHelper inspectContextIfInvalidatedInDebugMode:context]
#else
#define QDIM_CGContextInspectContext(context) if(![QDIMHelper inspectContextIfInvalidatedInReleaseMode:context]){return nil;}
#endif

typedef NS_ENUM(NSInteger, QDIMImageShape) {
    QDIMImageShapeOval,                 // 椭圆
    QDIMImageShapeTriangle,             // 三角形
    QDIMImageShapeDisclosureIndicator,  // 列表 cell 右边的箭头
    QDIMImageShapeCheckmark,            // 列表 cell 右边的checkmark
    QDIMImageShapeDetailButtonImage,    // 列表 cell 右边的 i 按钮图片
    QDIMImageShapeNavBack,              // 返回按钮的箭头
    QDIMImageShapeNavClose              // 导航栏的关闭icon
};

typedef NS_OPTIONS(NSInteger, QDIMImageBorderPosition) {
    QDIMImageBorderPositionAll      = 0,
    QDIMImageBorderPositionTop      = 1 << 0,
    QDIMImageBorderPositionLeft     = 1 << 1,
    QDIMImageBorderPositionBottom   = 1 << 2,
    QDIMImageBorderPositionRight    = 1 << 3,
};

@interface UIImage (QDIM)

/// 获取当前图片的像素大小，如果是多倍图，会被放大到一倍来算
@property(nonatomic, assign, readonly) CGSize qdim_sizeInPixel;
 ...
 *
 *  @return 处理完的图片
 *  @see qdim_imageResizedInLimitedSize:contentMode:scale:
 */
 ...
 *  @return 处理完的图片
 */
- (UIImage *)qdim_imageWithOrientation:(UIImageOrientation)direction;
