...
# scriptSelfPath: ./BaiYouSDK
#echo "---${PROJECT_DIR}"

#ENV: MacOS
#脚本路径：$BYCompanyProject/BaiYouSDK/Script
#由.podspec配置文件执行，需指定模块名称，执行EXP： `./autoLocalizedScript.sh BYShareKit`

set -e
cd BaiYouSDK/Classes
BYROOT_PATH=`pwd`
BYMODEL_PATH=""
...
function helpInfo() {
    ...
    echo -e "\nMacOS下,ERROR:无效参数,请传入正确的模块名称;\n  模块名称: ${BYROOT_PATH}路径下,一级文件夹名称；\n  EXP: ./autoLocalizedScript.sh BYShareKit\n"
    return 1;
}
    ...
    if [ -d "${BYMODEL_PATH}" ];then
        ...
        if [[ ${TARGET_NUM} -ge 2 ]]; then
            ...
            if [[ ${TARGET_NUM} -eq 0 ]]; then
                mkdir -p ${BYMODEL_PATH}/Resource/zh-Hans.lproj
                mkdir -p ${BYMODEL_PATH}/Resource/en.lproj
            fi
